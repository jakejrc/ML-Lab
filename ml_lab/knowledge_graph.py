...
def plot_knowledge_graph(G, figsize=(16, 12), layout_seed=42):
    ...
    for node_id, (x, y) in pos.items():
        ...
        ax.text(x, y, label, fontsize=8, ha=.center., va=.center., fontweight=.bold., color=.white.,
                bbox=dict(boxstyle=.round,pad=0.25., facecolor=.#1e293b., edgecolor=.none., alpha=0.75))
    legend_patches = []
...
const label = g.append('g').selectAll('text').data(nodesData).join('text')
    .text(d => d.label).attr('font-size', '9px').attr('font-weight', 'bold')
